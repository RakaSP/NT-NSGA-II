...
def _kmeans_pp_init(
    ...
    """
    k-means++ init.
    If K == 1 and fixed_first_index is not None, centroid is fixed at depot.
    """
    N = len(X)
    if rng is None:
...
def _kmeans(
    ...
    """
    Basic Lloyd k-means.
    All randomness (for empty-cluster reseeding) goes through `rng`.
    """
    ...
    for _ in range(max_iter):
        ...
        for k in range(K):
            if np.any(np.isnan(Cnew[k])):
                valid = np.array([i for i in range(K) if not np.any(np.isnan(Cnew[i]))], int)
                if valid.size == 0:
                    Cnew[k] = X[rng.integers(len(X))]
                else:
                    d2 = np.min(
                        np.linalg.norm(X[:, None, :] - Cnew[None, valid, :], axis=2) ** 2,
                        axis=1,
                    )
...
def cluster_nodes_azam(
    ...
    *,
    ...
    """
    Azam-style clustering (ID-based), capacity-free:
      1) k-means with depot fixed as a centroid (K=num_depots).
      2) depot per cluster = node closest to centroid.
      3) if vehicles > clusters: iteratively split clusters via 2-means with random seeding.
    Returns:
      {"clusters": List[List[int]], "depots": List[int]}
      (each cluster includes depot_id as first element)
    """
    ...
    if num_vehicles is not None and num_vehicles > len(clusters):
        ...
        for _ in range(need):
            weights = np.array([_complete_graph_weight_iddict(D, cl) for cl in clusters], dtype=float)
            total_w = weights.sum()
...
def build_cluster_subproblems(
    ...
    *,
    ...
    Pass through FULL D/T dicts so D[id_i][id_j] lookups remain valid.
    """
    nodes = vrp["nodes"]
    D_full = _get_D(vrp)
    T_full = _get_T(vrp)
